...
5. **index.md 업데이트**
   - 새로 생성된 페이지를 해당 카테고리 섹션에 추가한다.
   - 링크 형식: `[[wiki/<카테고리>/<파일명>|표시명]]` (명시적 경로 필수)
     - 예: `[[wiki/kafka/consumer-group|컨슈머-그룹]]`
     - 파일명은 영문 소문자 kebab-case, 표시명은 한글 또는 원래 명칭 사용
   - 항목 형식: `- [[wiki/<카테고리>/<파일명>|표시명]] — 한 줄 요약`
...
7. **log.md에 이력 추가**
   ...
   - 생성: [[wiki/<카테고리>/<파일명>|표시명]], ...
   - 업데이트: [[wiki/<카테고리>/<파일명>|표시명]], ...
   ```
